...
| PUT | `/api/auth/password` | 修改密码成功 | 修改本人密码 |
| GET | `/api/users` | 查询账号成功 | 账号列表 |
| POST | `/api/users` | 创建账号成功 | 创建账号 |
| PUT | `/api/users/{id}` | 编辑账号成功 | 编辑账号 |
| DELETE | `/api/users/{id}` | 删除账号成功 | 删除账号 |
| PUT | `/api/users/{id}/password` | 重置密码成功 | 超级管理员重置密码 |
| GET | `/api/shops` | 查询店铺成功 | 店铺列表 |
| POST | `/api/shops` | 创建店铺成功 | 创建店铺 |
| PUT | `/api/shops/{id}` | 编辑店铺成功 | 编辑店铺 |
| DELETE | `/api/shops/{id}` | 删除店铺成功 | 删除店铺 |
| GET | `/api/shops/{id}/customers` | 查询客户成功 | 店铺客户列表 |
| POST | `/api/shops/{id}/customers` | 创建客户成功 | 创建店铺客户 |
| PUT | `/api/shop-customers/{id}` | 编辑客户成功 | 编辑店铺客户 |
| DELETE | `/api/shop-customers/{id}` | 删除客户成功 | 删除店铺客户 |
...
| PUT | `/api/orders/{id}/review` | 设置问题件成功 | `action=SET_PROBLEM` |
| PUT | `/api/orders/{id}/review` | 设置其他成功 | `action=SET_OTHER` |
...
返回数据：分页 `ShopCustomerVO`。成功 message：`查询客户成功`。
...
返回数据：`ShopCustomerVO`。成功 message：`创建客户成功`。
...
返回数据：`ShopCustomerVO`。成功 message：`编辑客户成功`。
...
返回数据：`OrderVO`。成功 message 按 `action` 返回：`APPROVE` 为 `审核通过成功`，`REJECT` 为 `审核不通过成功`，`SET_PROBLEM` 为 `设置问题件成功`，`SET_OTHER` 为 `设置其他成功`。
...
- `SET_PROBLEM`：调度或超级管理员可手动设置为问题件。
- `SET_OTHER`：调度或超级管理员可手动设置为其他。
